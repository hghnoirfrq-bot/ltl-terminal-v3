...
const TerminalView = ({ setView }) => {
  ...
  const [output, setOutput] = useState([
    ...
    { type: 'line', text: 'System Status: Checking...' },
    ...
    <div>
      <div className="terminal-window">
        <div className="terminal-header">
          <span className="terminal-button red"></span><span className="terminal-button yellow"></span><span className="terminal-button green"></span>
        ...
        <div className="terminal-body" ref={terminalBodyRef}>
          ...
          <div className="command-line">
            ...
            <input type="text" className="terminal-input" autoFocus value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleCommand} />
            <span className="cursor"></span>
          </div>
        </div>
      </div>
      <div className="nav-cards">
        <div className="nav-card" onClick={() => setView('booking')}><h3>BOOK SESSION</h3><p>Schedule your creative coaching session. $75/hour.</p></div>
        <div className="nav-card" onClick={() => setView('login')}><h3>CLIENT PORTAL</h3><p>Access your dashboard, projects, and session history.</p></div>
        <div className="nav-card" onClick={() => setView('portfolio')}><h3>PORTFOLIO</h3><p>Explore LUCTHELEO's creative works.</p></div>
        <div className="nav-card" onClick={() => setView('stats')}><h3>SYSTEM STATUS</h3><p>Live analytics and system performance metrics.</p></div>
      </div>
    </div>
  );
};
